...
public class Apartment extends AppCompatActivity {
    private CheckBox apt1checkbox,apt2checkbox;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_apartment);
        apt1checkbox=findViewById(R.id.checkBox1);
        apt2checkbox=findViewById(R.id.checkBox2);
    }
    ...
    public void save_checked(View view){ //saving information from checkboxes to further process on checkout page
        ...
        boolean apt1state = apt1checkbox.isChecked();
        boolean apt2state = apt2checkbox.isChecked();
        prefEditor.putBoolean("apt1state",apt1state);
        prefEditor.putBoolean("apt2state",apt2state);
        prefEditor.commit();
